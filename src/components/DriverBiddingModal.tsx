...
export const DriverBiddingModal: React.FC<DriverBiddingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-5 sm:p-6 shadow-2xl space-y-5 text-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative py-4 flex flex-col items-center justify-center bg-slate-950/60 rounded-xl border border-slate-800 overflow-hidden">
          <div className="w-24 h-24 rounded-full border border-emerald-500/30 flex items-center justify-center relative">
            <div className="w-16 h-16 rounded-full border border-emerald-500/50 flex items-center justify-center">
              <Car className="w-8 h-8 text-emerald-400" />
            </div>
            <div className="absolute inset-0 rounded-full border-2 border-emerald-400/40 animate-ping opacity-75" />
          </div>
          <p className="text-xs font-semibold text-emerald-400 mt-2 flex items-center gap-1.5">
          ...
          {biddingDrivers.length === 0 ? (
            ...
            biddingDrivers.map((driver) => {
              ...
              return (
                <div
                  key={driver.id}
                  className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-emerald-500/80 transition-all flex items-center justify-between gap-3 shadow-md group"
                >
                  ...
                  <div className="text-right shrink-0">
                    <div className="mb-1">
                      <span className={`text-base font-black ${isCounterOffer ? 'text-amber-400' : 'text-emerald-400'}`}>
                        ৳ {driver.bidFareBDT}
                      </span>
                      {isCounterOffer && (
                        <p className="text-[9px] text-amber-300/80 font-medium">
