...
export const BookingPanel: React.FC<BookingPanelProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-xl space-y-5 text-white">
      ...
      <div className="space-y-3 relative">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={handleSwap}
            className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-all text-xs flex items-center gap-1 border border-slate-700"
        ...
        <div className="relative">
          <div className="flex items-center gap-3 p-3 bg-slate-950 border border-slate-800 rounded-xl focus-within:border-emerald-500 transition-all">
          ...
          {showPickupDropdown && (
            <div className="absolute top-full left-0 right-0 mt-1 z-30 max-h-48 overflow-y-auto bg-slate-950 border border-slate-800 rounded-xl shadow-2xl p-1 divide-y divide-slate-800/60">
              {filteredPickup.map((loc) => (
                <button
                  ...
                  className="w-full text-left p-2.5 hover:bg-slate-900 rounded-lg transition-all flex items-center justify-between group"
                >
                  <div>
                    <p className="text-xs font-bold text-slate-200 group-hover:text-emerald-400">
        ...
        {/* Dropoff Input */}
        <div className="relative">
          <div className="flex items-center gap-3 p-3 bg-slate-950 border border-slate-800 rounded-xl focus-within:border-rose-500 transition-all">
            <div className="w-4 h-4 rounded-full bg-rose-500 ring-4 ring-rose-500/20 shrink-0" />
            <input
              type="text"
          ...
          {showDropoffDropdown && (
            <div className="absolute top-full left-0 right-0 mt-1 z-30 max-h-48 overflow-y-auto bg-slate-950 border border-slate-800 rounded-xl shadow-2xl p-1 divide-y divide-slate-800/60">
              {filteredDropoff.map((loc) => (
                ...
                >
                  <div>
                    <p className="text-xs font-bold text-slate-200 group-hover:text-rose-400">
                      {language === 'bn' ? loc.nameBn : loc.name}
                    </p>
                    <p className="text-[10px] text-slate-500">
                      {language === 'bn' ? loc.addressBn : loc.address}
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 pt-1 no-scrollbar">
          ...
          {cityLocations.slice(0, 5).map((loc) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-slate-800/80 hover:bg-slate-700 text-slate-300 text-xs font-medium shrink-0 border border-slate-700/60 transition-all hover:scale-105"
            >
            ...
            return (
              <button
                ...
                className={`p-3 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col justify-between ${
                  isSelected
                    ? 'bg-gradient-to-br from-emerald-950 to-slate-900 border-emerald-500 shadow-md ring-1 ring-emerald-500/50'
      ...
      <div className="bg-slate-950 border border-amber-500/30 rounded-xl p-3.5 space-y-2">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="w-8 h-8 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm flex items-center justify-center border border-slate-700"
          >
            -
          </button>
          <input
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[
              { id: 'bkash', name: 'bKash', color: 'bg-pink-950 text-pink-300 border-pink-700' },
              { id: 'nagad', name: 'Nagad', color: 'bg-orange-950 text-orange-300 border-orange-700' },
              { id: 'cash', name: 'Cash', color: 'bg-emerald-950 text-emerald-300 border-emerald-700' },
            ].map((pm) => (
              <button
                key={pm.id}
                ...
                className={`py-2 text-xs font-bold rounded-lg border transition-all text-center ${
                  paymentMethod === pm.id
                    ? pm.color + ' ring-2 ring-emerald-500'
          ...
          <form onSubmit={handleApplyPromo} className="flex gap-1.5">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-8 pr-2 py-2 text-xs text-white uppercase focus:outline-none focus:border-amber-500"
              />
            </div>
            <button
              type="submit"
              className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-xs font-bold text-amber-300 rounded-lg border border-slate-700"
            >
      ...
      <button
        ...
        className={`w-full py-3.5 rounded-xl text-base font-black flex items-center justify-center gap-2 shadow-xl transition-all ${
          pickup && dropoff
            ? 'bg-gradient-to-r from-emerald-600 via-emerald-500 to-rose-600 hover:from-emerald-500 hover:to-rose-500 text-white shadow-emerald-950 active:scale-[0.99]'
