...
export const AIAssistantDrawer: React.FC<AIAssistantDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/70 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md bg-slate-900 border-l border-slate-800 h-full p-5 shadow-2xl flex flex-col justify-between text-white space-y-4">
        
        {/* Header */}
        <div>
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              <div className="p-2 rounded-xl bg-gradient-to-tr from-amber-500 to-emerald-500 text-slate-950 shadow-md">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg bg-slate-950 border border-slate-800 hover:border-amber-500/60 text-slate-300 text-xs transition-all text-left"
                >
                  💡 {q}
                </button>
              ))}
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex items-center gap-4 text-slate-400 pt-1">
                    ...
                    <span className="text-emerald-400 font-black">৳ {advice.estimatedFareBDT}</span>
                  </div>
                </div>
              </div>

              {/* Local Travel Tip Box */}
              <div className="bg-gradient-to-br from-amber-950/40 to-slate-950 border border-amber-500/30 rounded-xl p-3.5 text-xs text-amber-200 space-y-1">
        ...
        <form onSubmit={handleAskAI} className="space-y-2">
          <div className="relative">
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-3 pr-10 py-3 text-xs text-white focus:outline-none focus:border-amber-500"
            />
            <button
              type="submit"
              disabled={loading}
              className="absolute right-2 top-2 p-1.5 bg-gradient-to-r from-amber-500 to-emerald-500 hover:opacity-90 text-slate-950 rounded-lg font-bold"
            >
              {loading ? (
